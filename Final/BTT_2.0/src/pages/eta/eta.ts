...
/*
  Method to retrive the route number from kereta.route
*/
  getRoute(){
    //cari dulu stasiun
  ...
  */
  calculateRoute(){
     ...
     for(let i=0;i<this.myTrain.route.length;i++){
         s=this.stasiun[this.myTrain.route[i]].name.replace(/\s+/g,'');
         if(s==this.Location){
           //console.log(this.stasiun[this.myTrain.route[i]].name);"Stasiun Bandung"
           stasiunawalidx =i;
         }
     }
     console.log("Dapet Destination?? "+this.Destination);
     for(let i=0;i<this.myTrain.route.length;i++){
       s=this.stasiun[this.myTrain.route[i]].name.replace(/\s+/g,'');
       //console.log(s+" IS EQUAL "+this.Destination);
  ...
  */
  calculateStopoverDistance(){
    this.calculateMyMovement();

    //WAIT TILL GET LOCATION FIRST!
    setTimeout(() =>
    {
      ...
      for (let i = 0; i < this.stationDistances.length; i++) {
        ...
        //0 itu artinya sudah dilewati
        if(this.stopoverDistance[i] < 0){
          this.stopoverDistance[i] = 0;
        }
        console.log("STOPOVER "+this.stopoverDistance[i]);
      }
      //actual distance
      this.d = this.stopoverDistance[this.stopoverDistance.length-1];
    },
  ...
  /*
  This method is used to calculate the distance from our location to
  certain station.
  YOU HAVE TO GET THE LOCATION FIRST BEFORE PROGRESS ANY FURTHER!
  */
  calculateMyMovement(){
    var aa : any = 0.00;
    var cc : any = 0.00;
    ...
    //Dapetin kereta hasil milih
    //this.myTrain = this.kereta[0];
    this.myTrain = this.getMyTrain();

    //get my geolocation then
    //count it distance to spesific station
    this.geolocation.getCurrentPosition().then((resp) => {
    console.log(resp.coords.latitude, resp.coords.longitude);
    ...
    lonn = resp.coords.longitude;

    //our starting location
    //it will change everytime past through stopover station.
    //need global index to keep the information that tell this variable
    //later on that this variable will change if past through a station.
    //ex: the idx become route[1]
    var idx = this.getRoute();
    ...
    //save the changes of our distance.
    this.ourDistance = Number(dd);
    console.log(this.ourDistance+" away from previous station")
...
/*
Print out the information on ETA.
*/
  ...
  setTimeout(() =>  {
    ...
    for (let i = 0; i < this.ruteKereta.length; i++) {
      ...
      if(this.speed<=0){
        ...
        etas = this.stopoverDistance[i]/this.speed;
         ...
         //console.log(Date);
          ...
          this.localNotifications.schedule({
                id: i,
                text: 'Hey Wake Up',
                trigger: {at: new Date(new Date().getTime() + ((etas*3600)-(600) ) )},
                led: 'FF0000',
                sound: this.setSound(),
                //data: { secret:key }
          });
      ...
      this.items.push({
        jam: etas.toFixed(1) + ' Jam',
        jarak: this.stopoverDistance[i].toFixed(2) + ' Km',
